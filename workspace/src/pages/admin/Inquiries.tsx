  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        <div>
          <div className="text-2xl font-semibold text-zinc-50">询价管理</div>
          <div className="mt-2 text-sm text-zinc-400">
            代打 {byType.service} · 产品 {byType.product}
          </div>
        </div>
        <div className="inline-flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-300">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="text-sm font-semibold text-zinc-50">{i.request.name}</div>
                    <div className="text-xs text-zinc-400">{i.request.email}</div>
                  ...
                  <div className="mt-2 text-xs text-zinc-500">
                    {new Date(i.createdAt).toLocaleString()} · 线索 {i.id}
                  </div>
                  {i.quoteEstimate ? (
                  ...
                  {i.request.notes ? (
                    <div className="mt-2 line-clamp-2 text-xs text-zinc-400">{i.request.notes}</div>
                  ) : null}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="rounded-md border border-zinc-800 bg-zinc-950 px-2 py-2 text-sm text-zinc-200 outline-none focus:border-zinc-700"
                  >
                    {statuses.map((s) => (
