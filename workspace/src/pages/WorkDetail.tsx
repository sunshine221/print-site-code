...
export default function WorkDetail() {
  ...
  usePageTitle(detail.data?.title ? `${detail.data.title}` : "产品详情")
        ...
        {detail.loading ? (
          <div className="mt-6 grid gap-6 md:grid-cols-12">
            <div className="md:col-span-7">
              <div className="aspect-[4/3] animate-pulse rounded-2xl border border-zinc-200 bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-900/30" />
            </div>
            <div className="md:col-span-5">
              <div className="h-64 animate-pulse rounded-2xl border border-zinc-200 bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-900/30" />
            </div>
          </div>
        ) : detail.error ? (
          <div className="mt-6 rounded-xl border border-zinc-200 bg-white p-4 text-sm text-rose-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-rose-300">
            {detail.error}
          </div>
        ) : !detail.data ? null : (
          <div className="mt-6 grid gap-6 md:grid-cols-12">
            <div className="md:col-span-7">
              <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                ...
                {detail.data.media.length > 1 ? (
                  <div className="grid grid-cols-4 gap-2 border-t border-zinc-200 p-3 dark:border-zinc-800">
                    {detail.data.media.slice(0, 4).map((m) => (
                      <div
                        key={m.id}
                        className="aspect-[4/3] overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900"
                      >
            ...
            <div className="md:col-span-5">
              <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
                <div className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{detail.data.title}</div>
                <div className="mt-3 text-sm leading-relaxed text-zinc-600 dark:text-zinc-300">{detail.data.summary}</div>

                <div className="mt-5 flex flex-wrap gap-2 text-[11px] text-zinc-500 dark:text-zinc-400">
                  {detail.data.process ? (
                ...
                <div className="mt-6">
                  <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">选择 SKU</div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            selected
                              ? "border-zinc-300 bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-900/40"
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2 text-sm font-semibold text-zinc-900 dark:text-zinc-50">
                              {price}
                              <span className="text-[11px] text-zinc-500 dark:text-zinc-400">元</span>
                            </div>
                          </div>
                        </button>
                      )
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      sku?.id
                        ...
                        : "pointer-events-none bg-zinc-200 text-zinc-400 dark:bg-zinc-800 dark:text-zinc-400",
                    )}
                  >
                    立即询价
