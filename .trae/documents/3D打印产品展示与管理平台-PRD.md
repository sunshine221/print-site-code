...
<<<<<<< HEAD
## 1.1 目标与指标（可度量）
- 转化：询价提交转化率（提交成功 / 进入询价页）≥ X%
- 线索效率：管理员从“新建”到“首次处理”的中位耗时 ≤ X 小时
- 内容效率：新增/编辑产品从进入后台到发布 ≤ X 分钟
- 性能：首屏 LCP ≤ 2.5s（桌面），≤ 3.5s（移动）

## 1.2 范围与非范围
### 范围（本平台需要解决）
- 对外展示：官网首页、服务能力说明、产品/案例展示
- 转化闭环：询价/联系（双路径），可选附件上传，提交成功反馈
- 对内管理：管理员登录、内容维护、询价线索跟进

### 非范围（明确不做/暂不做）
- 在线支付、订单、发票等电商交易链路
- 多角色协作、复杂权限体系（默认单管理员/少量管理员）
- 客户侧账号体系（访客无需登录）

## 1.3 术语表
- 询价：访客提交的需求线索（可能包含附件与预估报价）
- 产品：对外展示的产品条目
- SKU：产品的规格组合（属性、参考价区间、可选库存等）
- 案例：展示类内容，可关联产品

## 1.4 需求追踪（闭环索引）
说明：为便于单人开发闭环，本 PRD 将需求映射到“需求卡片（含验收标准/边界用例/测试与发布）”。
- 需求卡片目录：[workspace/docs/project/features](file:///d:/Wysen/print_site_code/print-site-code/workspace/docs/project/features/README.md)
- Backlog：[workspace/docs/project/backlog.md](file:///d:/Wysen/print_site_code/print-site-code/workspace/docs/project/backlog.md)
=======
## 1.1 本次迭代目标（2026-05）
1. 提供白天/黑夜/跟随系统的页面模式，覆盖前台与后台
2. 管理后台登录态更稳定：已登录未登出时，前后台切换不需要重新登录
3. 补齐后台管理能力：产品管理、报价规则、询价管理均提供完整的增删改查
4. 修复关键交互与路由问题：仪表盘快捷入口跳转、文件上传后提交、后台菜单页面无响应等
5. 优化文案与转化路径：优先引导浏览平台产品，代打服务入口不抢占注意力
...
6. **页面模式**：白天/黑夜/跟随系统（默认跟随系统，支持手动切换并持久化）

### 2.2.1 可交付拆分（需求卡片）
- FE-001 官网首页（对应上表“官网首页”所有模块）：`workspace/docs/project/features/FE-001-官网首页.md`
- FE-002 代打服务详情：`workspace/docs/project/features/FE-002-代打服务详情.md`
- FE-003 产品与案例库（列表+详情）：`workspace/docs/project/features/FE-003-产品与案例库.md`
- FE-004 询价/联系（双路径）：`workspace/docs/project/features/FE-004-询价与联系.md`
- AD-001 管理员登录与鉴权：`workspace/docs/project/features/AD-001-管理员登录与鉴权.md`
- AD-002 内容管理（产品/SKU/案例/素材）：`workspace/docs/project/features/AD-002-内容管理.md`
- AD-003 询价线索管理：`workspace/docs/project/features/AD-003-询价线索管理.md`
- AD-004 报价规则与站点设置：`workspace/docs/project/features/AD-004-报价规则与站点设置.md`
...
| 询价/联系 | 提交成功页 | 订单编号、预计响应时间、补充信息入口（可选）、邮件回执（可选） |
| 后台管理 | 登录 | 管理员登录、忘记密码（可选）、登录态保持 |
| 后台管理 | 仪表盘 | 关键指标：本周询价量、未处理询价、热门产品、最近更新 |
| 后台管理 | 产品与SKU管理 | 产品CRUD、SKU CRUD（规格/属性/参考价/库存/上下架）、排序、标签、参数模板、媒体关联与封面设置、SEO字段（标题/描述） |
| 后台管理 | 案例管理 | CRUD、置顶、关联产品、媒体与文案 |
| 后台管理 | 素材库 | 图片/视频/3D文件管理、复用与引用、删除保护（被引用提示） |
| 后台管理 | 询价管理 | 线索列表、详情查看、状态流转（新建/处理中/已报价/已关闭）、预估价/实际报价记录（可选）、备注与跟进记录、删除/归档（可选） |
| 后台管理 | 报价规则 | 规则CRUD、启用/停用、材料/工艺/精度/数量/交期的加价系数与阶梯规则；规则版本与生效范围（可选） |
...
### 4.1.1 页面模式（白天/黑夜/跟随系统）
- 默认：跟随系统（prefers-color-scheme）
- 手动切换：白天 / 黑夜 / 跟随系统三档
- 持久化：同一浏览器在前台与后台均生效（localStorage 保存用户选择）
- 入口位置：导航栏右侧（前台）与后台顶部栏（后台）
...
### 4.5 文案与转化路径（本次优化）
1. 优先引导浏览平台产品：首页与导航的主CTA指向“平台产品/产品库”，代打服务入口弱化为次级入口
2. 询价类型入口文案：
   - 代打服务：需要上传模型文件
   - 平台产品：选择现有产品，不需要上传文件
3. 提交成功页：不使用“线索编号”措辞，统一展示为“订单编号”

## 7. 交互与校验要求（本次新增）
1. 代打服务询价表单必须校验：询价人信息（至少邮箱或电话其一）、数量（正整数）、交期（可选但格式正确）、文件上传（必填）等
2. 平台产品询价表单必须校验：必须选择产品SKU；询价人信息（至少邮箱或电话其一）
3. 管理后台登录支持两种输入：账号或邮箱 + 密码（输入框不默认提示具体示例账号/邮箱）
4. 上传文件完成后，上传区域应以“文件卡片/文件项”方式展示（文件名、大小、移除/替换）
...
## 5. 非功能性需求
### 5.1 性能
- 首屏加载快；媒体资源按需加载；列表与详情有骨架屏与占位状态
- 列表分页/懒加载，避免一次性渲染过多

### 5.2 可用性
- 错误提示明确；关键操作有状态反馈（loading/success/fail）
- 后台关键操作有确认与撤销（可选）

### 5.3 安全
- 后台必须鉴权；上传文件校验类型与大小
- 敏感信息不在前端明文存储；日志不输出敏感字段

### 5.4 SEO
- 公开页面支持可配置的标题/描述/开放图
- 产品详情具备可分享链接与可读的 meta 信息
